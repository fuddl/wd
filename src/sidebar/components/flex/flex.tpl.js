	...
	for (let x in iterator) {
		...
		for (let y in yAxis) {
			...
			for (let form of vars.forms) {
				let hyphenisation =
					form?.claims?.P5279?.[0]?.mainsnak?.datavalue?.value;
				...
				if (
					...
					if (
						...
						for (let rep in form.representations) {
							...
							if (
								hyphenisation &&
								hyphenisation.replace(/‧/g, "") === formStr
							) {
								formStr = hyphenisation.replace(
									/‧/g,
									String.fromCodePoint(173)
								);
							}
				...
				if (xAxis[x].present) {
					...
					for (let form of vars.forms) {
						if (
							...
							(form.grammaticalFeatures.includes(y) || !hasYAxis)
						) {
							let formLink = document.createElement("a");
							formLink.setAttribute("href", "#" + form.id);
